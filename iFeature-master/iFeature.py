...
if __name__ == '__main__':
	...
	userDefinedOrder = args.userDefinedOrder if args.userDefinedOrder != None else 'ACDEFGHIKLMNPQRSTVWXYU'
	userDefinedOrder = re.sub('[^ACDEFGHIKLMNPQRSTVWXYU]', '', userDefinedOrder)
	if len(userDefinedOrder) != 20:
		userDefinedOrder = 'ACDEFGHIKLMNPQRSTVWXYU'
	myAAorder = {
		'alphabetically': 'ACDEFGHIKLMNPQRSTVWYU',
		'polarity': 'DENKRQHSGTAPYVMCWIFL',
		'sideChainVolume': 'GASDPCTNEVHQILMKRFYW',
		'userDefined': userDefinedOrder
	}
	myOrder = myAAorder[args.order] if args.order != None else 'ACDEFGHIKLMNPQRSTVWXY'
